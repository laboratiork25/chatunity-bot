...
export async function handler(chatUpdate) {
  ...
  const isOwner = (() => {
    try {
      const baseOwners = [this.decodeJid(global.conn.user.id), ...global.owner.map(([number]) => number)]
        .filter(Boolean)
        .map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net')
      return baseOwners.includes(m.sender) || m.fromMe
    } catch {
      return false
    }
  })()
  ...
  try {
    ...
    async function isUserAdmin(conn, chatId, senderId) {
      try {
        ...
        return parts.some(p =>
          (conn.decodeJid(p.id || p.jid || '') === decodedSender) &&
    ...
    for (let name in global.plugins) {
      ...
      if ((usedPrefix = (match[0] || '')[0])) {
        ...
        if ((m.chat in global.db.data.chats || m.sender in global.db.data.users)) {
          let chat = global.db.data.chats[m.chat]
          let userX = global.db.data.users[m.sender]
          if (name != 'owner-unbanchat.js' && chat?.isBanned) return
          if (name != 'owner-unbanuser.js' && userX?.banned) return
        }
        ...
        try {
          ...
          if (!isPrems) {
            m.limit = m.limit || plugin.limit || 0
            m.money = m.money || plugin.money || 0
          }
        } catch (e) {
          m.error = e
          console.error(e)
...
export async function groupsUpdate(groupsUpdate) {
  if (opts['self']) return
  for (const groupUpdate of (Array.isArray(groupsUpdate) ? groupsUpdate : [])) {
    const id = groupUpdate.id
    if (!id) continue
    let chats = global.db.data.chats[id] || {}
    let text = ''
    if (groupUpdate.icon) text = (chats.sIcon || this.sIcon || global.conn.sIcon || '`immagine modificata`').replace('@icon', groupUpdate.icon)
    if (groupUpdate.revoke) text = (chats.sRevoke || this.sRevoke || global.conn.sRevoke || '`link reimpostato, nuovo link:`\n@revoke').replace('@revoke', groupUpdate.revoke)
    if (!text) continue
...
export async function callUpdate(callUpdate) {
  ...
  for (let nk of (Array.isArray(callUpdate) ? callUpdate : [])) {
...
global.dfail = (type, m, conn) => {
  let msg = {
    ...
    restrict: '🔐 𝐑𝐞𝐬𝐭𝐫𝐢𝐜𝐭 𝐞̀ 𝐝𝐢𝐬𝐚𝐭𝐭𝐢𝐯𝐚𝐭𝐨 🔐'
  }[type]
