...
export default async function (m, conn = { user: {} }) {
  try {
    ...
    // Decode del sender usando decodeJid
    let resolvedSender = conn.decodeJid ? conn.decodeJid(sender) : sender
    
    // Se è ancora un LID, prova con senderPn
...
${chalk.hex('#FE0041').bold('╰★────★────★────★────★')}`)
    ...
    // ⭐ GESTIONE DEL TESTO CON MENTION DECODIFICATI
    if (typeof m.text === 'string' && m.text) {
      let displayText = m.text
      
      // Se ci sono mention, decodificali
      if (m.mentionedJid && Array.isArray(m.mentionedJid) && m.mentionedJid.length > 0) {
        for (const id of m.mentionedJid) {
          try {
            ...
            // ⭐ SE È UN GRUPPO E IL JID È UN LID, CERCA IL VERO JID NEI PARTECIPANTI
            if (m.isGroup && /@lid/.test(mentionJid)) {
              try {
                ...
                // Se troviamo il partecipante con un JID reale, usalo
                ...
                // Ignora errori nel recupero metadati gruppo
              }
            }
    ...
    if (m.messageStubParameters?.length > 0) {
      const decoded = await Promise.all(m.messageStubParameters.map(async jid => {
        ...
        // Se è ancora un LID, prova con getPNForLID
