...
setInterval(async () => {
  if (stopped === 'close' || !global.conn || !global.conn.user) return;
  purgeSession(`./${global.authFile || 'sessioni'}`, true);
  const subBotDir = `./${global.authFileJB || 'chatunity-sub'}`;
  if (existsSync(subBotDir)) {
    const subBotFolders = readdirSync(subBotDir).filter(file => statSync(join(subBotDir, file)).isDirectory());
...
// ✅ FIX DEFINITIVO: Console filter RINVIATO dopo tutti gli import
...
// ✅ FIXED: Console filter APPLICATO ORA (dopo tutti gli import)
const filterStrings = [
    ...
    if (!global.conn.authState.creds.registered) {
      ...
      setTimeout(async () => {
        try {
          ...
          console.error('❌ Errore pairing:', err);
        }
      }, 3000);
    }
  }
